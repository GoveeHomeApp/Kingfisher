...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" Kingfisher.podspec`
...
LineNumber=`grep -nE 's.version.*=' Kingfisher.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" Kingfisher.podspec
echo -e "\n${Default}================================================"
